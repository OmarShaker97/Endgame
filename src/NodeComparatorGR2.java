...
public class NodeComparatorGR2 implements Comparator<Node> {
	...
	public int compare(Node n1, Node n2) {
		...
		byte totalDamageN1 = (byte) (((EndGameState)n1.getState()).getCoordinates().split(";")[2].length() * 3 + Endgame.thanosDamage);
		byte totalDamageN2 = (byte) (((EndGameState)n2.getState()).getCoordinates().split(";")[2].length() * 3 + Endgame.thanosDamage);
